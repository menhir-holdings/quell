...
The big picture is the case to execute. Reveal the algo when you need it; check the compounded last-layer after that algo on the cube you already have. Next always chains. If the case still has no name, Next asks for one before moving on.

The name sits in the diagram box, just above the picture. Click it to rename — there is no edit chrome.

### Flow
- **OLL / PLL** — last layer only. Always chained.
- **Top** — the case. **3D** — the cube in your hands.
- **Algo** — tap to reveal the moves; tap again to play.
- **After this algo** — tap to see the last layer you should have.
- **Change** — jump to any case in the current set.
- **Menu** — local accounts. Names (and later training stats) live per account.

Algos follow [SolveTheCube](https://solvethecube.com/algorithms): easy to memoise and turn, built around sexy `(R U R' U')`, sledge `(R' F R F')`, and `(R U R' U)`.

Progress lives in the browser (`localStorage` + IndexedDB) so names survive deploys on the same origin. Space = next, R = reveal algo.
...
| ID | Title | Status |
|----|-------|--------|
| [MT-198](https://linear.app/menhir-holdings/issue/MT-198) | Name in diagram, Change case, persist names, algo copy | In progress |
...
- Static algo data + localStorage / IndexedDB accounts
